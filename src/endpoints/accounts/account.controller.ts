...
export class AccountController {
  ...
  async getAccountsCount(
    ...
    return await this.accountService.getAccountsCount(
      new AccountQueryOptions({ ownerAddress, isSmartContract }),
    );
  }
  ...
  async getAccountVerification(
    ...
    try {
      ...
      this.logger.error(
        `Error in getAccountVerification for address ${address}`,
      );
      this.logger.error(error);
  ...
  async getAccountTokens(
    ...
    @Query('timestamp', ParseIntPipe) _timestamp?: number,
    @Query('mexPairType', new ParseEnumArrayPipe(MexPairType))
  ...
  async getTokenCount(
    ...
    try {
      return await this.tokenService.getTokenCountForAddress(
        address,
        new TokenFilter({
          type,
          search,
  ...
  async getAccountCollectionsWithRoles(
    ...
    @Query('canUpdateAttributes', new ParseBoolPipe())
  ...
  async getCollectionWithRolesCount(
    ...
    return await this.collectionService.getCollectionCountForAddressWithRoles(
      address,
      new CollectionFilter({
        search,
        type,
  ...
  async getAccountCollection(
    ...
    const result = await this.collectionService.getCollectionForAddressWithRole(
      address,
      collection,
    );
    if (!result) {
  ...
  async getAccountTokensWithRoles(
    ...
    return await this.tokenService.getTokensWithRolesForAddress(
      address,
  ...
  async getTokensWithRolesCount(
    ...
    return await this.tokenService.getTokensWithRolesForAddressCount(
      address,
      new TokenWithRolesFilter({
        search,
        owner,
  ...
  @ApiOperation({
    ...
    description:
      'Returns details about fungible token roles where the account is owner or has some special roles assigned to it',
  })
  @ApiOkResponse({ type: TokenWithRoles })
  async getTokenWithRoles(
    ...
    const result = await this.tokenService.getTokenWithRolesForAddress(
      address,
      identifier,
    );
    if (!result) {
      throw new NotFoundException(
        'Token with roles for given account not found',
      );
    }
  ...
  @ApiOperation({
    summary: 'Account collections',
    description:
      'Returns NFT/SFT/MetaESDT collections where the account owns one or more NFTs',
  })
  @ApiQuery({
    name: 'from',
    description: 'Number of items to skip for the result set',
  ...
  async getAccountNftCollections(
    ...
    return await this.collectionService.getCollectionsForAddress(
      address,
      new CollectionFilter({ search, type, excludeMetaESDT }),
      new QueryPagination({ from, size }),
    );
  }
  ...
  async getNftCollectionCount(
    ...
    return await this.collectionService.getCollectionCountForAddress(
      address,
      new CollectionFilter({ search, type, excludeMetaESDT }),
    );
  }
  ...
  async getAccountNftCollection(
    ...
    const result = await this.collectionService.getCollectionForAddress(
      address,
      collection,
    );
    if (!result) {
  ...
  async getAccountNfts(
    ...
    const options = NftQueryOptions.enforceScamInfoFlag(
      size,
      new NftQueryOptions({ withSupply, withScamInfo, computeScamInfo }),
    );
  ...
  async getNftCount(
    ...
    return await this.nftService.getNftCountForAddress(
      address,
      new NftFilter({
        search,
        identifiers,
  ...
  async getAccountNft(
    ...
    const result = await this.nftService.getNftForAddress(
      address,
      nft,
      actualFields,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Summarizes all delegation positions with staking providers, together with unDelegation positions',
  })
  ...
  async getAccountKeys(
    ...
    @Query('from', new DefaultValuePipe(0), ParseIntPipe) from: number,
    @Query('size', new DefaultValuePipe(25), ParseIntPipe) size: number,
    @Query('status', new ParseEnumArrayPipe(NodeStatusRaw))
  ...
  async getAccountTransactions(
    ...
    @Query(
      'function',
      new ParseArrayPipe(new ParseArrayPipeOptions({ allowEmptyString: true })),
    )
    functions?: string[],
    ...
    const options = TransactionQueryOptions.applyDefaultOptions(size, {
      withScResults,
      withOperations,
      withLogs,
      withScamInfo,
  ...
  getAccountContracts(
    ...
    return this.accountService.getAccountContracts(
      new QueryPagination({ from, size }),
      address,
    );
  }
  ...
  @ApiExcludeEndpoint()
  getAccountContractsCountAlternative(
    @Param('address', ParseAddressPipe) address: string,
  ): Promise<number> {
    return this.accountService.getAccountContractsCount(address);
  }
  ...
  getContractUpgrades(
    ...
    return this.accountService.getContractUpgrades(
      new QueryPagination({ from, size }),
      address,
    );
  }
  ...
  getAccountScResults(
    ...
    return this.scResultService.getAccountScResults(
      address,
      new QueryPagination({ from, size }),
    );
  }
  ...
  async getAccountScResult(
    ...
    if (
      !scResult ||
      (scResult.sender !== address && scResult.receiver !== address)
    ) {
  ...
  async getAccountTokenHistoryCount(
    ...
    const isToken =
      (await this.tokenService.isToken(tokenIdentifier)) ||
      (await this.collectionService.isCollection(tokenIdentifier)) ||
      (await this.nftService.isNft(tokenIdentifier));
    if (!isToken) {
